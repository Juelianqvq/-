...
def processCora(content_path, cite_path):
    ...
    paper_dict = dict([(int(key), int(val)) for val, key in enumerate(paper_list)])
    # 将八个分类标签存储在列表中
...
def main():
    # 数据集路径
    content_path = "cora.content"  # 节点属性信息
    cite_path = "cora.cites"  # 论文引用信息
    """
...
def modelStart(A):
    ...
    for epoch in range(200):
            ...
            loss = loss_function(out, label_list[:500].long())
